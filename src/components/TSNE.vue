<template lang="">
  <div id="container">
    <h2>t-SNE：t-SNE降维法</h2>
    <h3>t-SNE算法是一种非线性算法，全名为t分布随机邻域嵌入（t-distribud stocha neighbor eming，t-SNE）， 是一种用于探索高维数据的非线性降维机器学习算法。它将多维数据映射到适合于人类观察的两个或多个维度。PCA是一种线性算法，它不能解释特征之间的复杂多项式关系。而t-SNE是基于在邻域图上随机游走的概率分布来找到数据内的结构。线性降维算法的一个主要问题是不相似的数据点放置在较低维度表示为相距甚远，但为了在低维度用非线性流形表示高维数据，相似数据点必须表示为非常靠近，这不是线性降维算法所能做到的。</h3>
...
export default {
  ...
  methods: {
    ...
    submitUpload() {
      ...
      const url = 'TSNE' + '/' + this.input +'/'+ Math.random()
    ...
    submitDownload() {
      ...
      this.flag = 0
      window.open('http://120.27.210.217:8025/' +'TSNE' + '/download/' + Math.random())
      this.fileList = []
      this.input = ''
    }
  }
