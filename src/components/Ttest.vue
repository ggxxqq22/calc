<template lang="">
  <div id="container">
    <h2>单样本t检验：</h2>
    <h3>单样本t检验用于分析定量数据是否与某个常数数字有着显著的差异性。单样本是指研究者从关心的总体中抽样而得到的一组具有代表性的样本。</h3>
    <el-input v-model="input" placeholder="请输入待检验参数p"></el-input>
    <el-upload
      class="upload"
      ref="upload"
      action="none"
...
export default {
  ...
  methods: {
    ...
    submitUpload() {
      //判断是否有正确的argv
      if(this.input == ''){
        this.$alert('请输入正确的待检验参数p后再上传！', '上传错误', {
      ...
      const url = 'Ttest' + '/' + this.input +'/'+ Math.random()
    ...
    submitDownload() {
      ...
      this.flag = 0
      window.open('http://120.27.210.217:8025/' +'Ttest' + '/download/' + Math.random())
      this.fileList = []
      this.input = ''
    }
  }
