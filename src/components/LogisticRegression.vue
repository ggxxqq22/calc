<template lang="">
  <div id="container">
    <h2>逻辑回归：逻辑回归处理分类问题</h2>
    <h3>Logistic Regression虽然被称为回归，但其实际上是分类模型，并常用于二分类。Logistic Regression因其简单、可并行化、可解释化深受工业界喜爱。 逻辑回归的本质是：假设数据服从这个分布，然后使用极大似然估计做参数的估计。</h3>
    <el-upload
      class="upload"
      ref="upload"
      action="none"
...
export default {
  ...
  methods: {
    ...
    submitUpload() {
      ...
      const url = 'logisticRegression' + '/' + 'upload' +'/'+ Math.random()
    ...
    submitDownload() {
      ...
      this.flag = 0
      window.open('http://120.27.210.217:8025/' + 'logisticRegression' + '/download/' + Math.random())
      this.fileList = []
      this.input = ''
    }
  }
