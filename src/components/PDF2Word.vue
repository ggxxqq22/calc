<template lang="">
  <div id="container">
    <h2>PDF：文献PDF内容提取</h2>
    <h3>将PDF格式的文献或其他文件通过python的pdfminer库转换为word格式的文件</h3>
    <el-upload
      class="upload"
      ref="upload"
      action="none"
      ...
      <div slot="tip" class="el-upload__tip">请上传一个文件，只支持上传pdf文件</div>
    </el-upload>
    <div class="download">
...
export default {
  ...
  methods: {
    ...
    submitUpload() {
      ...
      const url = "pdf2word" + "/" + "upload" + "/" + Math.random();
    ...
    submitDownload() {
      ...
      this.flag = 0;
      window.open(
        "http://120.27.210.217:8025/" + "pdf2word" + "/download/" + Math.random()
      );
      this.fileList = [];
      this.input = "";
    },
