<template lang="">
  <div id="container">
    <h2>聚类：K-means聚类</h2>
    <h3>将物理或抽象对象的集合分成由类似的对象组成的多个类的过程被称为聚类。这里聚类采用Kmeans聚类算法来实现，Kmeans聚类算法又称k均值聚类算法，其是一种迭代求解的聚类分析算法， 其步骤是随机选取K个对象为初始的聚类中心，然后计算每个对象与各个种子聚类中心之间的距离，把每个对象分配给距离它最近的聚类中心。聚类中心以及分配给它们的对象就代表一个聚类。没分配一个样本，聚类的聚类中心会根据聚类中现有的对象被重新计算。这个过程将不断重复直到满足某个终止条件。</h3>
    <el-input v-model="input" placeholder="请输入聚类数量k"></el-input>
    <el-upload
      class="upload"
      ref="upload"
      action="none"
  ...
  methods: {
    ...
    submitUpload() {
      ...
      if(!(this.input > 0 && this.input < 100)){
        this.$alert('请输入正确的聚类数量k后再上传！', '上传错误', {
      ...
      const url = 'kmeans' + '/' + this.input +'/'+ Math.random()
    ...
    submitDownload() {
      ...
      this.flag = 0
      window.open('http://120.27.210.217:8025/' +'kmeans' + '/download/' + Math.random())
      this.fileList = []
      this.input = ''
    }
  }
